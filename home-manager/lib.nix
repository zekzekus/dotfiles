...
let
  ...
  mkHmModule = host: {
    ...
    home-manager.users.${host.home.username} =
      { ... }:
      {
        imports = host.home.modules;
      };
    home-manager.extraSpecialArgs = host.home.specialArgs;
  };
  ...
  mkHomeConfiguration =
    { hostname, system, extraModules ? [], extraSpecialArgs ? {} }:
    let
      host = mkHost { inherit hostname system extraModules extraSpecialArgs; };
      pkgs = import nixpkgs {
